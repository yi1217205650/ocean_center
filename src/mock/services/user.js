...
import { UID } from '@/store/mutation-types'
import Mock from 'mockjs2'
import { builder, getQueryParameters } from '../util'

const info = options => {
  const userIndex = storage.get(UID) - 10001
  const userinfoList = [
    {
      name: '管理员',
      username: 'admin',
  ...
  const userInfo = {
    id: Mock.mock('@id'),
    name: userinfoList[userIndex].name,
    username: userinfoList[userIndex].username,
    avatar: userinfoList[userIndex].avatar,
    password: '',
    status: 1,
    telephone: '',
    ...
    deleted: 0,
    roleId: userinfoList[userIndex].roleId,
    role: {}
  }
  // role
  const adminRoleObj = {
  ...
  const roleList = [adminRoleObj, adminRoleObj, customerRoleObj, customerRoleObj1]
  userInfo.role = roleList[userIndex]
...
const userList = (options) => {
  ...
  for (let i = 1; i < next; i++) {
    result.push({
      ...
      pwd: Mock.mock('@string("lower", 5, 8)'),
      role: Mock.mock('@integer(0, ' + (roleList.length - 1) + ')'),
